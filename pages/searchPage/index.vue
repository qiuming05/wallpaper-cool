<template>
	<view>
		...
		<view class="main" :style="{paddingTop: navbarHeight +'px'}">
			<imgList :data="data" :isload="isload" :height="'calc(100vh - ' + navbarHeight  + 'px)'" refresherEnabled
...
<script>
	...
	export default {
		...
		methods: {
			...
			getData() {
				return this.$request({
					url: baseUrl + `search?${objToUrl(store.state.$screen)}&page=${this.page}`
				})
			},
			scrolltolower() {
				if (this.isload) {
